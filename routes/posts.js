...
router.get("/", isLoggedIn, (req, res) => {
    User.findById(req.user._id) // 친구들의 게시물
        .populate({
            path: "friends",
            populate: {
                path: "posts",
...
router.get("/post/:id/like", isLoggedIn, (req, res) => {
    User.findById(req.user._id, (userErr, user) => {
        if (userErr) {
            console.log(userErr);
            req.flash(
                "There has been an error trying to like this post. are you looged in?"
            );
            res.redirect("back");
        } else {
...
router.get("/post/:postid/comments/:commentid/like", isLoggedIn, (req, res) => {
    User.findById(req.user._id, (userErr, user) => {
        if (userErr) {
            ...
            User.findById(req.params.commentid, (commentErr, comment) => {
                if (commentErr) {
                    ...
                    comment.save();
                    user.liked_posts.push(comment._id);
                    user.save();
                    req.flash(
                        "success",
